...
#define NPTS \
  8  // number of points for the linear
...
AAF sin(const AAF& P) {
  if (P.is_infinite()) return AAF(interval(-1, 1));
...
AAF cos(const AAF& P) { return sin(P + PI / 2); }
...
AAF tan(const AAF& P) { return sin(P) / cos(P); }
...
AAF cotan(const AAF& P) { return cos(P) / sin(P); }

// Hyperbolics

AAF cosh(const AAF& P) { return (exp(P) + exp(-P)) / 2; }

AAF sinh(const AAF& P) { return (exp(P) - exp(-P)) / 2; }
...
AAF acosh(const AAF& P) { return log(P + sqrt(sqr(P) - 1)); }

AAF asinh(const AAF& P) {
  if (P.get_center() < 0) return -asinh(-P);
  return (log(P + sqrt(sqr(P) + 1)));
}

AAF atanh(const AAF& P) { return {}; }

...
AAF::AAF(const AAF &P) : special(P.special) {
...
double AAF::get_center() const { return cvalue; }
...
interval AAF::convert() const {
  ...
  if (is_indeterminate()) return interval(-HUGE_VAL, HUGE_VAL);
...
AAF half_plane(const AAF &P) {
  ...
  AAF_TYPE type;
  if (P.special == AAF_TYPE_NAN) return P;
...
double AAF::index_coeff(unsigned index) const {
  for (int i = 0; i < length; i++)
    if (indexes[i] == index) return coefficients[i];
  return 0;
}

...
interval::interval() : lo(0), hi(0) {}

interval::interval(double x) : lo(x), hi(x) {}
...
interval interval::operator*(const interval &i) const {
  return {glm::min(lo * i.lo, lo * i.hi, hi * i.lo, hi * i.hi),
          glm::max(lo * i.lo, lo * i.hi, hi * i.lo, hi * i.hi)};
}

// Modify the lower bound of an interval

void interval::mod_lo(const double low) { lo = low; }

// Modify the higher bound of an interval

void interval::mod_hi(const double high) { hi = high; }
...
void interval::int_vprint() const { printf("[%f,%f]\n", lo, hi); }

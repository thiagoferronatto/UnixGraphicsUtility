...
class AAF {
 ...
 public:
  ...
  AAF_TYPE get_special() const { return special; }
};
...
inline AAF::AAF(AAF_TYPE t)
    : special(t), cvalue(0), length(0), coefficients(NULL), indexes(NULL) {}

// Says highest symbol in use is val

inline void AAF::set_default(const unsigned val) { last = val; }

// Increment the highest symbol in use
// i.e. create a new noise symbol

inline unsigned AAF::inclast() { return ++last; }

// Get the length of an AAF
// i.e the number of non-null noise symbols

inline unsigned AAF::get_length() const { return length; }
...
inline bool AAF::straddles_zero() const { return convert().straddles_zero(); }

inline bool AAF::strictly_neg() const { return convert().right() < 0; }
